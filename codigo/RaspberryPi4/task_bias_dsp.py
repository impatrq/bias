...
def procesar_senales_eeg():
    n = 512
    fs = 256.0
    eeg_signals = {1: np.random.randn(n), 2: np.random.randn(n), 3: np.random.randn(n), 4: np.random.randn(n)}
    print("Procesando señales EEG...")

    for channel, signal in eeg_signals.items():
        time.sleep(1)
        print(f"Señal {channel} procesada.")
    print("Procesamiento de señales EEG completado.")
...
if __name__ == "__main__":
    ...
    thread_eeg = threading.Thread(target=procesar_senales_eeg)
    thread_eeg.start()
